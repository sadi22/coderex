...
class Index extends Component {
    static async getInitialProps(context) {
        ...
        let per_page = page_number === 1 ? 11 : 10;
        let apiMethod = page_number === 1 ? getDataByEndPoint('wp/v2/posts?offset=1') : getDataByEndPoint('wp/v2/posts');
    
        try {
            const [page, posts, recentPost] = await Promise.all([
                await getPostBySlug('blog').then(data=>{return data[0]}),
                apiMethod.then(data => {
                    return data;
                }),
                await getDataByEndPoint('wp/v2/posts?offset=0&per_page=1').then(data => {
                    return data[0];
                })
            ]);
            return { page, posts, recentPost, page_number };
        } catch (err) {
            console.log(err);
        }
        return null;
    ...
    render() {
        const { page, posts, siteOptions, recentPost, page_number } = this.props;
        ...
        let activePage = parseInt(page_number);
        ...
        return (
               <Fragment>
                   <Head>
                    ...
                    <div className="blog">
                        {activePage === 1 ?
                            <section className="inner-page-banner blog-banner pos-relative">
                                        ...
                                        <div className="cr-col">
                                            <div className="banner-content align-col-left-bottom">
                                                ...
                                                <div className="sub-title-wrapper">
                                                    {recentPost.category_info && 
                                                        recentPost.category_info.map((term, index) => {
                                                            ...
                                                            return (
                                                                <Link
                                                                    ...
                                                                    key={index}
                                                                >
                                                                    <a className="circular-700 color-fff text-uppercase tag">{ (index ? ', ' : '') + term_name} </a>
                                                                </Link>
                                                    ...
                                                    <Link
                                                        as={`/post/${recentPost.slug}`}
                                                        href={`/post/[slug]`}
                                                    >
                                                        <a><h1 className="sub-title circular-700">{recentPost.title.rendered}</h1></a>
